...
async function handler (request, response, verb) {
 ...
 switch(digits) {
  ...
  default:
    await response.say('Ha insertado una respuesta incorrecta. Las respuestas válidas son:  UNO para responder con SÍ. Y CERO para responder con NO.', { name: "es-US-Standard-B" });
...
const personas = main.selectPersonas();
...
personas.then(async function(result) { 
        ...
        result.forEach(auxPersona => {
          ...
          const nombrePersona = auxPersona.nombrecompleto_persona;
          ...
          voiceServer.listen(async(req, res) => {
           ...
           //await res.say("You said " + speech);
            ...
            await res.say("Hola, esta llamada corresponde con una encuesta para conocer el estado de salud luego de recibir su vacuna contra el COVID 19. " , { name: "es-US-Standard-B"});
        
            ciudadano = "Es la persona respondiendo esta encuesta el ciudadano "+nombrePersona;
            await res.say(ciudadano+ `.  Si su respuesta es SÍ. marque uno.  Si es otra persona asistiendo al ciudadano `+nombrePersona +`. Marque el cero `, { name: "es-US-Standard-B"});
            ...
            await res.say('Hola '+nombrePersona+ `. PARA ESTA Y TODAS LAS PREGUNTAS DE LA ENCUESTA, SI DESEAS RESPONDER CON UN: SI. MARCA UNO.  SI TU RESPUESTA US UN:  NO. MARCA EL CERO.` + fiebre, { name: "es-US-Standard-B" }); 
          ...
          console.log('salio');
                ...
                main.selectPhone(auxPersona).then(result => { 
                
                call.call(result[0].movil_contacto.replace(/-/g,''));
